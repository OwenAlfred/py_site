        ...
        if ret:
            for l in ret:
                house = {
                    ...
                    "img_url":conf.HOUSE_IMG_URL_PREFIX + l["hi_index_image_url"] if l["hi_index_image_url"] else ""
                }
                houses.append(house)
...
class HouseImageHandler(BaseHandler):
    ...
    def post(self):
        ...
        house_img_oriname = self.request.files['house_image'][0]['filename']
         # 获取房屋图片的扩展名
        try:
            name_suffix = house_img_oriname[house_img_oriname.rindex('.') : ]
        except Exception as e:
            logging.error(e)
            name_suffix = '.jpg'

        # 为上传的房屋图片生成唯一的名字
        house_img_rename = uuid.uuid4().hex

        # 保存上传的房屋图片到服务器
        try:
            with open('%s%s%s' % (conf.HOUSE_IMG_URL_PREFIX, house_img_rename, name_suffix), 'wb') as w:
                w.write(house_image)
        except Exception as e:
            logging.error(e)
            return self.write({"errcode":RET.THIRDERR, "errmsg":"image storage error"})
        ...
        try:
            ...
            self.db.execute(sql, house_id=house_id, url=house_img_rename+name_suffix)
        except Exception as e:
            logging.error(e)
            return self.write({"errcode":RET.DBERR, "errmsg":"第一句upload failed"})

        try:
            sstence = "update ih_house_info set hi_index_image_url=%(url)s " \
                        "where hi_house_id=%(house_id)s and hi_index_image_url is null;"
        except Exception as e:
            logging.error(e)
            return self.write({"errcode":RET.DBERR, "errmsg":"第二句upload failed"})


        img_url = conf.HOUSE_IMG_URL_PREFIX + house_img_rename + name_suffix
...
class HouseInfoHandler(BaseHandler):
    ...
    def post(self):
        ...
        facility = self.json_dict.get("facility") # 对一个房屋的设施，是列表类型
        # 校验
    ...
    def get(self):
        ...
        data = {
            ...
            "user_avatar":conf.AVATAR_URL_PREFIX + ret["up_avatar"] if ret.get("up_avatar") else ""
        }
        ...
        if ret:
            for image in ret:
                ...
                images.append(conf.HOUSE_IMG_URL_PREFIX + image["hi_url"])
        data["images"] = images
        ...
        if ret:
            ...
            for l in house_ret:
                ...
                house = {
                    ...
                    "img_url": conf.HOUSE_IMG_URL_PREFIX + l["hi_index_image_url"]
                }
                houses.append(house)
        ...
        if ret:
            for l in ret:
                house = dict(
                    ...
                    avatar=conf.AVATAR_URL_PREFIX+l["up_avatar"] if l.get("up_avatar") else "",
                    # image_url=constants.QINIU_URL_PREFIX+l["hi_index_image_url"] if l.get("hi_index_image_url") else ""
                    image_url=conf.HOUSE_IMG_URL_PREFIX+l["hi_index_image_url"] if l.get("hi_index_image_url") else ""
                )
                data.append(house)
