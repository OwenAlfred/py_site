...
class REST:
    ...
    def CreateSubAccount(self, friendlyName):
        ...
        auth = base64.encodestring(src.encode('utf8')).strip()
        req = urllib.request.Request(url)
        self.setHttpHeader(req)
        req.add_header("Authorization", auth)
        #xml格式
        body ='''<?xml version="1.0" encoding="utf-8"?><TemplateSMS><appId>%s</appId>\
    ...
    def sendTemplateSMS(self, to,datas,tempId):
        ...
        req = urllib.request.Request(url, body.encode(encoding='utf-8'), {"Accept":"application/xml","Content-Type":"application/xml;charset=utf-8","Authorization":auth})
        # print(dir(req))
        print('req.get_method()是%s '%req.get_method())
        print('header信息： %s'%req.header_items())
        '''
        会显示False，因为urllib在构建Request对象时，对headers的key做了Captialize,
        "Content-Type"的key变成了"Content-type",处理 header 后，将其赋予一个成员变量 headers 。
        因此，如果是在构建Request对象之后需要设置header,你只需要重新替换掉headers这个成员就可以了。
        '''
